...
<p align="center">
  <em>HiveReach 是 <a href="https://github.com/Panniantong/Agent-Reach">Panniantong/Agent-Reach</a> (MIT) 的独立维护 fork，自 2026-05-05 起独立演化。</em>
</p>
...
```
帮我安装 HiveReach：https://raw.githubusercontent.com/xavierliang/HiveReach/main/docs/install.md
```
...
| 🩺 **自带诊断** | `hivereach doctor` 一条命令告诉你哪个通、哪个不通、怎么修 |
...
>
> HiveReach 依赖 Agent 执行 shell 命令（`pip install`、`mcporter`、`twitter` 等）。如果你的 OpenClaw 使用了默认的 `messaging` 工具配置，Agent 将无法执行命令。**安装前请先开启 exec 权限**：
>
> ```bash
...
> ```
> 帮我安装 HiveReach（安全模式）：https://raw.githubusercontent.com/xavierliang/HiveReach/main/docs/install.md
> 安装时使用 --safe 参数
> ```
...
1. **安装 CLI 工具** — `pip install` 装好 `hivereach` 命令行
...
安装完之后，`hivereach doctor` 一条命令告诉你每个渠道的状态。
</details>
...
**HiveReach 是一个脚手架（scaffolding），不是框架。**

你给一个新 Agent 装环境的时候，总要花时间去找工具、装依赖、调配置——Twitter 用什么读？Reddit 怎么绕封？YouTube 字幕怎么提取？每次都要重新踩一遍。

HiveReach 做的事情很简单：**帮你把这些选型和配置的活儿做完了。**

安装完成后，Agent 直接调用上游工具（twitter-cli、rdt-cli、xhs-cli、yt-dlp、mcporter、gh CLI 等），不需要经过 HiveReach 的包装层。
...
每个渠道文件只负责检测对应上游工具是否可用（`check()` 方法），给 `hivereach doctor` 提供状态信息。实际的读取和搜索由 Agent 直接调用上游工具完成。
...
所以从 HiveReach 的视角看，它依然只是一个 `mcporter` 里的 `douyin` server，只是能力更完整。
...
| 措施 | 说明 |
|------|------|
| 🔒 **凭据本地存储** | Cookie、Token 只存在你本机 `~/.hivereach/config.yaml`，文件权限 600（仅所有者可读写），不上传不外传 |
| 🛡️ **安全模式** | `hivereach install --safe` 不会自动修改系统，只列出需要什么，由你决定装不装 |
| 👀 **完全开源** | 代码透明，随时可审查。所有依赖工具也是开源项目 |
| 🔍 **Dry Run** | `hivereach install --dry-run` 预览所有操作，不做任何改动 |
...
HiveReach 调用各平台公开 API/网页时会带浏览器风格 `User-Agent`，这意味着遵守平台 ToS 和速率限制是**你**的责任。详见 [docs/ethical-use.md](docs/ethical-use.md)。

### 📦 安装方式

| 方式 | 命令 | 适合场景 |
|------|------|---------|
| 一键全自动（默认） | `hivereach install --env=auto` | 个人电脑、开发环境 |
| 安全模式 | `hivereach install --env=auto --safe` | 生产服务器、多人共用机器 |
| 仅预览 | `hivereach install --env=auto --dry-run` | 先看看会做什么 |
...
会清除：`~/.hivereach/`（含所有 token/cookie）、各 Agent 的 skill 文件、mcporter 中的 MCP 配置。

```bash
# 只预览，不实际删除
hivereach uninstall --dry-run

# 只删 skill 文件，保留 token 配置（重装时用）
hivereach uninstall --keep-config
```

卸载 Python 包本身：`pip uninstall hivereach`

---

## 贡献

这个项目是纯 vibe coding 出来的 🎸 可能会有一些不完美的地方，如果遇到问题请多多包涵。有 bug 尽管提 [Issue](https://github.com/xavierliang/HiveReach/issues)，我都会尽快修复。

**想要新渠道？** 直接提 Issue 告诉我们，或者自己提 PR。

**想在本地加？** 让你的 Agent clone 下来改就行，每个渠道就是一个独立文件，加起来很简单。

[PR](https://github.com/xavierliang/HiveReach/pulls) 也随时欢迎！
...
HiveReach 使用 [twitter-cli](https://github.com/public-clis/twitter-cli) 通过 Cookie 认证访问 Twitter，完全免费。安装：`pipx install twitter-cli`，确保浏览器已登录 x.com，Agent 就可以用 `twitter search "关键词"` 搜索、`twitter tweet URL` 读推文了。
</details>

<details>
<summary><strong>How to search Twitter/X with AI agent for free (no API)?</strong></summary>

HiveReach uses twitter-cli with cookie auth — zero API fees. Install with `pipx install twitter-cli`, make sure you're logged into x.com in your browser, then your agent can search with `twitter search "query"` and read tweets with `twitter tweet URL`.
</details>

<details>
<summary><strong>Reddit 返回 403 怎么办？</strong></summary>

HiveReach 使用 [rdt-cli](https://github.com/public-clis/rdt-cli) 访问 Reddit，无需登录、无需代理、无需 API Key。安装：`pipx install rdt-cli`。Agent 可以用 `rdt search "关键词"` 搜索、`rdt read POST_ID` 读帖子全文和评论。
</details>
...
Yes! HiveReach is an installer + configuration tool — any AI coding agent that can run shell commands can use it. Works with Claude Code, Cursor, OpenClaw, Windsurf, Codex, and more. Just `pip install hivereach`, run `hivereach install`, and the agent can start using the upstream tools immediately.
...
> Bug 反馈和功能请求请用 [GitHub Issues](https://github.com/xavierliang/HiveReach/issues)，更容易跟踪。
...
[腾讯云 OpenClaw](https://www.tencentcloud.com/act/pro/intl-openclaw?referral_code=G76Y819A&lang=zh&pg=) — 在腾讯云Lighthouse秒级部署OpenClaw全能助手，可通过对话丝滑接入HiveReach，给你的OpenClaw一键装上互联网能力。
