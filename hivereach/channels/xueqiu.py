...
class XueqiuChannel(Channel):
    ...
    def __init__(self):
        # Per-instance cookie state — keeps tests isolated and avoids
        # surprising cross-instance pollution if multiple channels are
        # constructed (e.g. one per agent session).
    ...
    def _load_cookies_from_config(self) -> bool:
    ...
    def _ensure_cookies(self) -> None:
        ...
        if self._cookies_initialized:
            return
        if self._load_cookies_from_config():
